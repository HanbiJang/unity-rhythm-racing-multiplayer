...
public class PlayerCameraController : MonoBehaviour
{
    ...
    void FindPlayer()
    {
        ...
        {
            // PlayerFollower 컴포넌트로 먼저 찾기 (더 확실함)
            PlayerFollower playerFollower = FindObjectOfType<PlayerFollower>();
            if (playerFollower != null)
            {
                m_PlayerTransform = playerFollower.transform;
                m_PlayerFound = true;
                Debug.Log($"PlayerCameraController: PlayerFollower를 찾았습니다. 위치: {m_PlayerTransform.position}");
            }
            else
            {
                // Player 태그로 찾기
                ...
                if (playerObj != null)
                {
                    ...
                    m_PlayerFound = true;
                    Debug.Log($"PlayerCameraController: Player 태그로 플레이어를 찾았습니다. 위치: {m_PlayerTransform.position}");
                }
                else
                {
                    Debug.LogWarning($"PlayerCameraController: 플레이어를 찾을 수 없습니다. {m_FindPlayerRetryInterval}초 후 다시 시도합니다.");
                    m_PlayerFound = false;
                    // 플레이어를 찾지 못했으면 주기적으로 재시도
